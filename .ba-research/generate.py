#!/usr/bin/env python3
"""Generate Barossa Valley TS data files from the research JSONs.

Reads .ba-research/*.json and emits data/regions/barossa-valley/
...
OUT = os.path.normpath(os.path.join(HERE, '..', 'data', 'regions', 'barossa-valley'))
...
def emit_experiences():
    ...
    header = ("import { Experience } from '../../../types';\n\n"
              "// ---------------------------------------------------------------------------\n"
              "// Barossa Valley — the full experiences catalogue.\n"
              ...
              "// ---------------------------------------------------------------------------\n\n")
    body = ("export const BAROSSA_EXPERIENCES_FULL: Experience[] = [\n" +
...
def split_mixed_batches(names):
    """Mixed-subregion batch files are routed to the right output file by
    each winery's subregion. Wines follow their winery."""
    area = {'Tanunda': 'valley', 'Marananga & Seppeltsfield': 'valley',
            'Nuriootpa & Greenock': 'north', 'Angaston': 'north',
            'Lyndoch & Williamstown': 'north', 'Eden Valley': 'eden'}
    out = {k: {'wineries': [], 'wines': []} for k in ('valley', 'north', 'eden')}
    for name in names:
        d = load(name)
        if not d:
            continue
...
if __name__ == '__main__':
    ...
    nw, nwi = emit_winery_file('wineries-valley.ts', 'VALLEY_WINERIES',
                               ...
                               [mixed['valley']])
    print(f'valley: {nw} wineries, {nwi} wines')
    ...
    nw, nwi = emit_winery_file('wineries-eden.ts', 'EDEN_WINERIES',
                               'Eden Valley — the high, cool country east of the floor.',
                               [mixed['eden']])
    print(f'eden: {nw} wineries, {nwi} wines')
